...
void R2D2AdvActor::reset(const HanabiEnv& env) {
...
void R2D2AdvActor::observeBeforeAct(const HanabiEnv& env) {
...
void R2D2AdvActor::act(HanabiEnv& env, const int curPlayer) {
  torch::NoGradGuard ng;
...
void R2D2AdvActor::fictAct(const HanabiEnv& env) {
  if (!offBelief_) {
    return;
  }
  torch::NoGradGuard ng;
...
void R2D2AdvActor::observeAfterAct(const HanabiEnv& env) {
  torch::NoGradGuard ng;
  if (replayBuffer_ == nullptr) {
    return;
  }
